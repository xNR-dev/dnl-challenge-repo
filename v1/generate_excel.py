...
def main():
    ...
    ws_overview.append([])
    for key in ["entity_type", "size_class", "reporting_framework", "pl_format", "applicable_date"]:
        ws_overview.append([key.replace("_", " ").title(), meta.get(key, "")])
    ws_overview.append(["Total Items", len(items)])
    ws_overview.append(["Mandatory", sum(1 for i in items if i["obligation"] == "M")])
    ws_overview.append(["Conditional", sum(1 for i in items if i["obligation"] == "C")])

    # --- Per-section sheets ---
    COLUMNS = [
        "ID", "Section", "Sub-section", "Checklist Item",
        "HGB Reference", "Obligation", "Trigger Condition",
        "Completeness Prompt", "Audit Response"
    ]
    ...
    for sec in sections_order:
        ...
        # Column widths
        widths = [10, 10, 20, 55, 22, 12, 30, 50, 14]
    ...
    widths = [10, 10, 20, 55, 22, 12, 30, 50, 14]
    for i, w in enumerate(widths, 1):
        ws_all.column_dimensions[chr(64 + i)].width = w

    wb.save(OUTPUT)
    print(f"Excel saved to {OUTPUT}")
    print(f"Total items: {len(items)}")
    for sec in sections_order:
        count = sum(1 for i in items if i["section"] == sec)
        print(f"  {sec}: {count}")
